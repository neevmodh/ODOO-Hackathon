...
// Simple JSON only response, no HTML output allowed.
...
if (!$data || !isset($data['question_id'], $data['vote_type'])) {
...
// For demo, vote on first answer of question
$res = $mysqli->query("SELECT id FROM answers WHERE question_id = $question_id LIMIT 1");
if ($res->num_rows === 0) {
    echo json_encode(['success' => false, 'message' => 'No answers to vote on']);
    exit;
}
